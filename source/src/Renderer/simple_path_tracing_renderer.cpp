#include "Renderer/simple_path_tracing_renderer.h"
#include "Util/frame.h"

glm::vec3 SimplePathTracingRenderer::renderPixel(const glm::ivec2& pixelCoord, RNG& rng)
{
    ...
    for (int bounce = 0;; ++bounce) {
        ...
        if (!hitInfo) {
            if (lastIsSpecular) {
                for (const auto& infiniteLight: scene_.getInfiniteLights()) {
                    ...
                    L += beta * infiniteLight->getEmission(ray.origin, ray.origin + scene_.getRadius() * 2 * lightDir, -lightDir);
                }
            }
            break;
        }
        ...
        if (lastIsSpecular && hitInfo->material && hitInfo->material->areaLight) {
            L += beta * hitInfo->material->areaLight->getEmission(ray.origin, hitInfo->hitPos, hitInfo->normal);
        }
        // BRDF = albedo / PI
        // PDF = cos_theta / PI
        ...
        if (hitInfo->material) {
            ...
            if (!lastIsSpecular) {
                auto lightSourceSample = scene_.getLightSampler().sample(rng.uniform(), false);
