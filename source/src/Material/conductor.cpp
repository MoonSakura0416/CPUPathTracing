                         ...
                         const RNG& rng)
{
    ...
    if (microfacet_.isDeltaDistribution()) {
        ...
        pdf = microfacet_.visibleNormalDistribution(wi,microfacetNormal) / (4.f * glm::abs(glm::dot(wi, microfacetNormal)));
        bsdf = fr * microfacet_.heightCorrelatedMaskingShadowing(lightDir, wi, microfacetNormal)
               * microfacet_.normalDistribution(microfacetNormal) / (4.f * glm::max(glm::abs(wi.y * lightDir.y),eps));
    }
