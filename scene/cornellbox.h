...
inline void CornellBox()
{
    ...
    // Scene  scene{};
    //
    // auto redMaterial = new Diffuse(glm::vec3{0.65f, 0.05f, 0.05f});
    // auto whiteMaterial = new Diffuse(glm::vec3{0.73f, 0.73f, 0.73f});
    // auto greenMaterial = new Diffuse(glm::vec3{0.15f, 0.60f, 0.18f});
    //
    //
    // // Left wall (red), x=0, normal direction +X
    // auto leftWall =
    //     new Quad(glm::vec3{0, 0, 0}, glm::vec3{0, 555, 0}, glm::vec3{0, 0, 555});
    // scene.addInstance(leftWall, redMaterial);
    //
    // // Right wall (green), x=555, normal direction -X
    // auto rightWall =
    //     new Quad(glm::vec3{555, 0, 0}, glm::vec3{0, 0, 555}, glm::vec3{0, 555, 0});
    // scene.addInstance(rightWall, greenMaterial);
    //
    // // Back wall (white), z=555, normal facing -Z
    // auto backWall =
    //     new Quad(glm::vec3{0, 0, 555}, glm::vec3{0, 555, 0}, glm::vec3{555, 0, 0});
    // scene.addInstance(backWall, whiteMaterial);
    //
    // // Floor (white), y=0, normal direction +Y
    // auto floor =
    //     new Quad(glm::vec3{0, 0, 0}, glm::vec3{0, 0, 555}, glm::vec3{555, 0, 0});
    // scene.addInstance(floor, whiteMaterial);
    //
    // // Ceiling (white), y=555, Normal facing -Y
    // auto ceiling =
    //     new Quad(glm::vec3{0, 555, 0}, glm::vec3{555, 0, 0}, glm::vec3{0, 0, 555});
    // scene.addInstance(ceiling, whiteMaterial);
    //
    // // Light source (on the ceiling), y=554, normal direction -Y
    // auto light = new Quad(glm::vec3{180.5, 554, 200.75}, glm::vec3{195, 0, 0},
    //                                     glm::vec3{0, 0, 157.5});
    // // Lighting material: It is black itself (not reflecting light), but it will shine automatically
    // auto lightMaterial = new Diffuse(glm::vec3{0.0f, 0.0f, 0.0f});
    // auto areaLight = new AreaLight({15.f / 2.f, 15.f / 2.f, 15.f / 2.f}, light, false);
    // scene.addAreaLight(areaLight, lightMaterial);
    // // lightMaterial->setEmission({15.f / 2.f, 15.f / 2.f, 15.f / 2.f});
    // //scene.addShape(light, lightMaterial);
    //
    //
    // auto glassMaterial = new Dielectric(1.5f, glm::vec3{1, 1, 1}, 0.1f, 0.1f);
    // auto sphere = new Sphere(glm::vec3{0, 0, 0}, 70.f);
    ...
    // auto      goldMaterial = new Conductor(goldEta, goldK, 0.25f, 0.25f);
    // auto      model = new Model("model/dragon.obj");
    ...
    //     pathTracingRenderer.render(5000, "cornellbox3.ppm");
    // }
}
