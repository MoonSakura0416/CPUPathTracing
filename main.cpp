...
#ifdef _DEBUG
    #define new new(_CRT_BLOCK, __FILE__, __LINE__)
#endif
...
int main()
{
    ...
    for (int i = -3; i <= 3; i++) {
        ...
        auto mat = scene.addMaterial<Conductor>(glm::vec3{2.f - c * 2.f},
    ...
    scene.addInstance(model, dragonMat1,
                   {-5, 0.4, 1.5}, {2, 2, 2});

    auto dragonMat2 = scene.addMaterial<Conductor>(glm::vec3{0.1, 1.2, 1.8}, glm::vec3{5, 2.5, 2}, 0.4, 0.4);
    scene.addInstance(
        model,
        dragonMat2,
        {-5, 0.4, -1.5}, {2, 2, 2});
